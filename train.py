...
from retrain_model.build_camonas import Retrain_CamoNAS
from config_utils.retrain_camonas_args import obtain_retrain_camonas_args
...
def main():
    ...
    args = obtain_retrain_camonas_args()
    model_fname = 'data/camonas_{0}_epoch%d.pth'.format(args.dataset)

    kwargs = {'num_workers': args.workers, 'pin_memory': True, 'drop_last': True}
    dataset_loader, num_classes = dataloaders.make_data_loader(args, **kwargs)
    args.num_classes = num_classes

    model = Retrain_CamoNAS(args)
    ...
    for epoch in range(start_epoch, args.epochs):
        ...
        for i, sample in enumerate(dataset_loader):
            ...
            print('epoch: {0}\titer: {1}/{2}\tlr: {3:.6f}\tloss: {loss.val:.4f} ({loss.ema:.4f})'
                  .format(epoch + 1, i + 1, len(dataset_loader), scheduler.get_lr(optimizer), loss=losses))
